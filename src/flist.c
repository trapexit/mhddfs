...
// add file to list
struct flist *
flist_create(const char *name, const char *real_name, int flags, int fh)
{
...
struct flist **
flist_items_by_eq_name(struct flist * info)
{
  struct flist * next;
  struct flist ** result;
  int i = 0, count = 0;
...
static struct flist *
item_by_id(uint64_t id, int wrlock)
{
  struct flist *next;
  if(wrlock)
    flist_wrlock();
...
struct flist *
flist_item_by_id_wrlock(uint64_t id)
{
...
/* internal function */
static void
delete_item(struct flist *item, int locked)
{
  struct flist *next;
...
void
flist_delete(struct flist *item)
{
...
void
flist_delete_locked(struct flist *item)
{
...
void
flist_delete_wrlocked(struct flist * item)
{
